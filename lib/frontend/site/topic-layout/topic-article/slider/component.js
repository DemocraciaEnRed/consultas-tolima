      ...
      <div className={`topics-slider ${colored !== false ? colored : 'neutral'}`}>
        {
          showRange && <div className='container-fluid'>
            <div className='input-great-labels'>
              <span>{t('proposal-result.very-against')}</span>
              <span>{t('proposal-result.very-support')}</span>
            </div>
            <InputRange
              maxValue={99}
              minValue={-99}
...
const fillFormatLabel = (value) => {
  const range = {
    '-99': t('proposal-result.very-against'),
    '-66': t('proposal-result.middle-against'),
    '-33': t('proposal-result.little-against'),
    '0': t('proposal-result.neutral'),
    '33': t('proposal-result.little-support'),
    '66': t('proposal-result.middle-support'),
    '99': t('proposal-result.very-support')
  }
      ...
      <div className='col-xs-12 col-md-6'>
        <VictoryChart
          ...
          <VictoryAxis
            ...
            label={t('proposal-result.very-against')} />
        </VictoryChart>
      </div>
      <div className='col-xs-12 col-md-6'>
        <VictoryChart
          ...
          <VictoryAxis
            style={{ axis: { visibility: 'hidden' }, tickLabels: { fontSize: 14 }, axisLabel: { fontSize: 24, padding: 100 } }}
            tickFormat={(t) => fillFormatLabel(t)}
            label={t('proposal-result.little-support')} />
        </VictoryChart>
      </div>
    </div>
  )
